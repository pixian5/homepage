...
| `history` | Chrome / Firefox（Safari 不声明，因 API 不支持） | 读取系统最近历史 | `app.js` 的 `loadRecentHistory()` |
| `tabs` | 三端 | Popup/后台打开链接；Safari 最近访问回退记录 | `popup.js` / `visit-history.js` |
...
5. Safari **无法**读取系统浏览历史（WebExtensions history API 不支持）。历史分组显示扩展自建的「最近访问」：请先在 Safari 正常浏览网页，再打开新标签页查看
6. Safari 请到「设置 → 扩展」确认「我的首页」已启用，且「网站」权限里「对于其他网站」为允许（用于图标与内容脚本）
...
- **Safari 不支持 WebExtensions `history` API**（平台限制）。本扩展在 Safari 使用「最近访问」回退：用 `tabs` 事件记录你浏览过的 http(s) 页面，显示在历史分组。**不是**系统完整浏览历史；需先在 Safari 打开一些网页后才会有数据。
