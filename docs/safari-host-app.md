...
- 新版本首次加载时，如果扩展 local storage 缺失或意外变空，而 App Group 副本非空，则自动恢复。
...
安装后必须运行 `verify --restore-on-regression`：若更新前存在 `homepage_data`，而更新后节点数或数据字节数减少，则自动把结构化快照写回当前签名身份的数据库，并让构建失败报警。这样构建不能再把“扩展安装成功但主页数据变空”报告成成功。
