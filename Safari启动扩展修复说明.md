...
- 新标签页覆盖：正常（⌘T）
- 新建窗口覆盖：正常
- 启动时覆盖：正常（Safari 设置为“启动时打开新窗口”，且“新建窗口时打开”选择本扩展）

> 注意：如果之前安装过旧 bundle id（`com.homepage.newtab.safari.extension`）的调试版本，Safari 扩展列表可能出现重复条目并导致覆盖页失效。构建脚本已自动清理该旧插件。
