...
- `PUT /v1/sync/state`
- ETag / `If-Match` 并发保护
- PUT 串行提交，落盘成功后才发布新内存状态
- 严格校验 `SyncDocument` 结构与 `contentHash`
- 原子写入 JSON 状态文件，启动时拒绝损坏或不一致的状态
- 可选 Bearer Token 鉴权
...
- 并发 PUT 在服务端按到达后的处理顺序串行执行，ETag 判断与落盘属于同一个提交区间；磁盘写入失败不会推进内存 revision。
- 服务端会拒绝哈希错误、集合类型错误、重复 ID、无效引用和非法时间字段，避免损坏文档进入其他客户端。
...
删除快捷项、文件夹或分组时，客户端会把原对象移到本机不可见的同步墓碑层，而不是立刻遗忘其 ID。墓碑会跟随同步投影上传；另一台设备即使仍保存较早的存活对象，合并时也会由删除时间戳裁决，已删除对象不会重新出现在页面上。

- 墓碑保留 60 天，写入本机数据或生成同步投影时自动清理过期记录，避免长期占用同步包空间。
- 在保留期内，较晚的编辑可以显式复活对象；撤销删除也会写入新的更新时间，以避免旧墓碑再次删除它。
- 删除所有分组和卡片同样会生成墓碑。若要让服务器立即以当前本机数据为准，可在设置中使用“本机覆盖云端”。
- 22.2 之前已物理删除、且未留下墓碑的历史删除无法凭空恢复其删除记录。升级后若旧项目已经复活，在任一最新版客户端重新删除并同步一次即可建立稳定的删除记录。
