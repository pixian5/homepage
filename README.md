...
| `history` | Chrome / Firefox / Safari | 读取最近历史记录 | `app.js` 的 `loadRecentHistory()` |
...
5. Safari 已声明 `history` 权限；若系统/浏览器未暴露 history API 或未授权，历史分组会显示“无法读取浏览器历史”提示，而不是静默空白
6. Safari 版本若历史仍为空，请到「设置 → 扩展」确认「我的首页」已启用，并允许相关权限后重新打开新标签页
...
- Safari 已声明 `history` 权限；若运行时仍无 `browser.history.search`，界面会提示「无法读取浏览器历史」（平台限制/未授权），而非程序崩溃。
